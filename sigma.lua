...
local function getObjGen()
    ...
    local function getObjects()
        local function initObj()
            ...
            Gui.MainUI.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
            ...
            Gui.Logo.ZIndex = 122
            Gui.Logo.Image = "rbxassetid://104764966647835"
...
function UILibrary.new(gameName, userId, rank)
    ...
    --// Customize the GUI
    window.Watermark.Text = ("niggasense beta | %s | %s"):format(userId, gameName)
...
-- Add this code to your existing library (sigma.lua)

-- Create a Frame for the feature status list
local featureListFrame = Instance.new("Frame")
featureListFrame.Size = UDim2.new(0.2, 0, 0.5, 0) -- Adjust size as needed
featureListFrame.Position = UDim2.new(0.8, 0, 0.1, 0) -- Position it on the screen
featureListFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
featureListFrame.Parent = yourMainUI -- Replace with your main UI reference

-- Table to keep track of feature statuses
local features = {
    Aimbot = false,
    ESP = false,
...
-- Function to update the feature list display
local function updateFeatureList()
    -- Clear previous entries
    for _, child in pairs(featureListFrame:GetChildren()) do
        if child:IsA("TextLabel") then
            child:Destroy()
        end
    end

    -- Create a new TextLabel for each feature
    for feature, isEnabled in pairs(features) do
        local label = Instance.new("TextLabel")
        label.Size = UDim2.new(1, 0, 0, 30) -- Adjust height as needed
        label.BackgroundTransparency = 1
        label.TextColor3 = Color3.fromRGB(255, 255, 255)
        label.Text = feature .. ": " .. (isEnabled and "On" or "Off")
        label.Parent = featureListFrame
    end
end
...
local function toggleFeature(feature)
    if features[feature] ~= nil then
        features[feature] = not features[feature] -- Toggle the feature status
        updateFeatureList() -- Update the display
    end
end

-- Example of toggling features
-- Call this function when you toggle aimbot
function UILibrary.Window:ToggleAimbot()
    toggleFeature("Aimbot")
    -- Your existing aimbot toggle logic here
end

function UILibrary.Window:ToggleESP()
    toggleFeature("ESP")
    -- Your existing ESP toggle logic here
end
